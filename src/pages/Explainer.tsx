...
export function ExplainerPage() {
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl font-bold text-white">Explainer Studio</h1>
        ...
        {!ready && (
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
            ...
            <span>
              ...
              <Link to="/rumor-desk" className="text-sky-400 hover:underline">Rumor Desk</Link>.
            </span>
          </div>
        )}
      </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    <button
                      ...
                      className={`rounded-full px-2.5 py-1 text-xs ring-1 ring-inset transition ${
                        on
                          ...
                          : 'bg-white/5 text-slate-500 line-through ring-white/10'
                      }`}
                    >
                      {e.name}
                    </button>
                ...
                {analysis.matched.length === 0 && (
                  <p className="text-xs text-slate-500">No known characters detected in the story.</p>
                )}
              </div>
            </div>
                  ...
                  return (
                    <li key={r.event.id}>
                      <label className="flex cursor-pointer items-start gap-2 text-xs text-slate-300">
                        ...
                        />
                        <span>
                          <span className="font-mono text-slate-500">{formatDate(r.event.date)}</span>{' '}
                          {r.event.title}{' '}
          ...
          <section>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex overflow-hidden rounded-full ring-1 ring-inset ring-white/15">
                <button
                  onClick={() => setFormat('article')}
                  className={`px-4 py-1.5 text-sm transition ${
                    format === 'article' ? 'bg-white/15 text-white' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Cited article
                </button>
                <button
                  ...
                  className={`px-4 py-1.5 text-sm transition ${
                    format === 'thread' ? 'bg-white/15 text-white' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
            ...
            {format === 'article' ? (
              <div className="mt-4 max-h-[70vh] overflow-y-auto rounded-xl border border-white/10 bg-slate-950/70 p-5">
                ...
                {explainer.tweets.map((t, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                      ...
                      <div className={`mt-2 text-right text-[11px] ${over ? 'text-rose-400' : 'text-slate-500'}`}>
