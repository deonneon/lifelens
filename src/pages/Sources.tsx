    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-ink-400">
        {Object.entries(SOURCE_TYPE_META).map(([key, meta]) => (
          <span key={key} className="rounded-full bg-surface-2 px-3 py-1 ring-1 ring-inset ring-edge" title={meta.blurb}>
          ...
          return (
            <div key={source.id} className="card">
              ...
              >
                ...
                <span className="font-display text-sm font-semibold text-ink-100">{source.title}</span>
              ...
              {isOpen && (
                <div className="border-t border-edge p-4">
                  {source.url && (
                    <a href={source.url} target="_blank" rel="noreferrer" className="text-xs text-accent-bright hover:underline">
                      {source.url} ↗
                    </a>
                  )}
                    ...
                    {accounts.map((a) => {
                      ...
                      return ev ? (
                        <li key={a.id} className="flex flex-wrap items-center gap-2 text-sm">
                          ...
                          <Link to={`/events/${ev.id}`} className="text-ink-100 hover:text-ink-50 hover:underline">
                            {ev.title}
                          </Link>
                          <span className="text-xs text-ink-500">({formatDate(ev.date)})</span>
                        </li>
                      ) : null
                    })}
                    {accounts.length === 0 && (
